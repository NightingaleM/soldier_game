...
export const SKILL_BOOK: {
    ...
    allWordAtk: (G: G) => {
        return {
            ...
            name: "领导全世界的一击",
            intro:
                "每次攻击有概率集合所有单位攻击进行全力一击。攻击等级越高概率越大，攻击速度等级越高概率越小。",
            type: "before_atk",
            effect: (S: HeroGenerator) => {
    ...
    ferment: (G: G) => {
        return {
            ...
            name: "酝酿",
            intro:
                "攻击完一次后，需要回味和学习。每次攻击有概率增加2%攻击力，同时增加2%的攻击间隔",
    ...
    fakeDJ: (G: G) => {
        return {
            ...
            name: "打碟咯",
            intro:
                "带节奏，隔不久就要带次节奏，一天天叭叭哒哒的。每次攻击，额外造成 攻击力 * 攻击间隔的伤害。",
            type: "before_atk",
            effect: (S: HeroGenerator) => {
        ...
        return {
            ...
            effect: (S: HeroGenerator) => {
                if (Math.random() < 0.2 && Math.random() > 0.8) {
                    ...
                    if (atk_level === spd_level && Math.random() < 0.4) {
                        // 1.6%
                        s.forEach((item) => {
                            item.UPGRADE_ATK({
                                withoutCost: true,
    ...
    commonProsperity: (G: G) => {
        return {
            ...
            name: "共同富裕",
            intro:
                "领袖带领队员们一同富裕。提升 攻击力等级相关的金币获取效率，降低攻速等级相关的金币花费",
            type: "after_upgrade",
            effect: (S: HeroGenerator) => {
                G.goldCoin.addMultiples({
                    name: "commonProsperity",
                    value: BigInt(S.atk_level),
                });
                G.goldCoin.cutMultiples({
                    name: "commonProsperity",
                    value: BigInt(S.spd_level),
                });
            },
        };
    },
    ...
    smallRedPacket: (G: G) => {
        return {
            ...
            name: "小红包",
            intro:
                "每挂机一分钟有机会偶遇一名已经激活得英雄，获得见面红包，金额为英雄的攻击力。",
            type: "global",
            effect: ({ skill, G, S }) => {
                const fn = () => {
    ...
    newYearRedPacket: (G: G) => {
        return {
            ...
            name: "新年红包",
            intro:
                "每挂机5分钟，获取新年红包，金额为所有等级比自己高得英雄攻击力的总和。",
            type: "global",
            effect: ({ skill, G, S }) => {
                const fn = () => {
                    G.timers[`${S.name}_${skill.id}`] = setTimeout(
